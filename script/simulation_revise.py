...
zodi = np.load('Data/zodi_0.npy')           # zodiacal light by wavelength(electron)


############### main ###############

def main():
    global sci_evaluation, sci_sigma
    ...
    sci_evaluation = np.array([[[0]*len(lamb)]*T_star_len]*n, float)
    ...
    sci_sigma = np.array([[[0]*len(lamb)]*T_star_len]*n, float)
    ...
    if wog==True:
        ...
        np.save('Result/sub_evaluation_both_{}_{}.npy'.format(pattern, volume), sub_evaluation_1)
        np.save('Result/sub_evaluation_back_{}_{}.npy'.format(pattern, volume), sub_evaluation_2)
        np.save('Result/sub_evaluation_ref_{}_{}.npy'.format(pattern, volume), sub_evaluation_3)
        np.save('Result/sci_sigma_{}_{}.npy'.format(pattern, volume), sci_sigma)
        np.save('Result/sub_sigma_both_{}_{}.npy'.format(pattern, volume), sub_sigma_1)
        np.save('Result/sub_sigma_back_{}_{}.npy'.format(pattern, volume), sub_sigma_2)
        np.save('Result/sub_sigma_ref_{}_{}.npy'.format(pattern, volume), sub_sigma_3)
    print('Success')
