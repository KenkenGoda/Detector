...
n = 100                                       # repeat times
T_star_len = 4
zodi_std = 500                               # standard deviation of zodiacal light(ppm)
...
def main():
    ...
    print('ZODI std: {}'.format(zodi_std))
    ...
    np.save('Result/sub_evaluation_both_variation_zodi_{}.npy'.format(zodi_std), sub_evaluation_1)
    np.save('Result/sub_evaluation_back_variation_zodi_{}.npy'.format(zodi_std), sub_evaluation_2)
    np.save('Result/sub_sigma_both_variation_zodi_{}.npy'.format(zodi_std), sub_sigma_1)
    np.save('Result/sub_sigma_back_variation_zodi_{}.npy'.format(zodi_std), sub_sigma_2)
    print('Success')


############### make the zodi fluctuation ###############

def zodi_fluctuation(TT):
    period = np.linspace(60*2, t_one[TT]*60*4, 10000)
    frequency = 1/period
    zodi_variation_one = zodi_fluctuation_one(frequency, TT)
    zodi_variation = zodi_variation_one
    for _ in range(times-1):
        zodi_variation_one = zodi_fluctuation_one(frequency, TT)
        zodi_variation = np.hstack((zodi_variation, zodi_variation_one))
    return zodi_variation

def zodi_fluctuation_one(frequency, TT):
    fluctuation = np.sum([zodi_Fluctuation(f, t0[TT]*60) for f in frequency], axis=0)
    fluctuation = fluctuation*(zodi_std*10**(-6)/fluctuation.std())
    return 1+fluctuation

def zodi_Fluctuation(f, t):
    return np.sin(2*np.pi*(f*t+rd.random()))
...
def science(TT, ll, gain, n_pix):
    ...
    D_sci_ijk = np.array([rd.normal((tra[kk]+zodi[ll][kk])/n_pix+Q,
                                   np.sqrt(((bose_star*tra[kk]+bose_zodi*zodi[ll][kk])/n_pix+Q+read)/(n_pix/n_gate)))
...
def background(TT, ll, gain, n_pix):
    x_zodi = (h*c)/(lamb[ll]*10**(-4)*k*275)
    bose_zodi = abs(np.exp(x_zodi)/(np.exp(x_zodi)-1))
    D_back_ijk = np.array([rd.normal(zodi[ll][kk]/n_pix+Q,
                                     np.sqrt(((bose_zodi*zodi[ll][kk])/n_pix+Q+read)/(n_pix/n_gate)))
                           for kk in range(t_total[TT])], float)*gain
    return D_back_ijk
...
def gain_calibration(nn, TT):
    ...
    for ll in range(len(lamb)):
        if lamb[ll]<lamb_bord[1]:
            D_back_all = np.mean(D_back_ave[np.where(lamb<lamb_bord[1])], axis=0)
            D_ref = D_ref_ave[0]
        elif lamb_bord[1]<=lamb[ll]<lamb_bord[2]:
            D_back_all = np.mean(D_back_ave[np.where((lamb>=lamb_bord[1])&(lamb<lamb_bord[2]))], axis=0)
            D_ref = D_ref_ave[1]
        else:
            D_back_all = np.mean(D_back_ave[np.where(lamb>=lamb_bord[2])], axis=0)
            D_ref = D_ref_ave[2]
...
def evaluation(nn, TT):
    global tra_one, z, zodi
    tra_one = np.load('Data/%s_%d.npy'%(pattern, T_star[TT]))
    z = np.load('Data/parameter_%d.npy'%T_star[TT])
    zodi_variation = zodi_fluctuation(TT)
    zodi = [np.load('Data/zodi.npy')[ll]*zodi_variation for ll in range(len(lamb))]             # zodiacal light by wavelength(electron)
