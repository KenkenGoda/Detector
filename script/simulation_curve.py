...
wog = True                                  # if the signal has gain (wog: without-gain)
...
def main():
    [evaluation(TT) for TT in range(T_star_len)]
    print('Success')
...
def unify(TT):
    ...
    for jj in range(n_gate):
        ...
        if wog==True:
            ...
            D_ref_ave_k[2, jj] = reference(TT, gain, ref_pix)
        else:
...
def evaluation(TT):
    ...
    D_sci, D_sub_1, D_sub_2, D_sub_3 = gain_calibration(TT)
    ...
    mod_curve = D_mod_norm
    sci_curve = [normalization(TT, tt, D_sci[ll]) for tt in range(times)]
    if wog==True:
        np.save('Result/wog_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), sci_curve)
    else:
        sub_both_curve = [normalization(TT, tt, D_sub_1[ll]) for tt in range(times)]
        sub_back_curve = [normalization(TT, tt, D_sub_2[ll]) for tt in range(times)]
        sub_ref_curve = [normalization(TT, tt, D_sub_3[ll]) for tt in range(times)]
        np.save('Result/mod_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), mod_curve)
        np.save('Result/sci_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), sci_curve)
        np.save('Result/sub_both_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), sub_both_curve)
        np.save('Result/sub_back_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), sub_back_curve)
        np.save('Result/sub_ref_curve_{}_{}.npy'.format(pattern, int(T_star[TT])), sub_ref_curve)
