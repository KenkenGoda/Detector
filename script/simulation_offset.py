...
offset_error = 50                            # ratio of offset error(%)
n = 100                                     # repeat times
T_star_len = 4
...
def main():
    ...
    print('offset error: {}%'.format(offset_error))
    ...
    np.save('Result/sub_evaluation_both_offset_{}.npy'.format(offset_error), sub_evaluation_1)
    np.save('Result/sub_evaluation_back_offset_{}.npy'.format(offset_error), sub_evaluation_2)
    np.save('Result/sub_evaluation_ref_offset_{}.npy'.format(offset_error), sub_evaluation_3)
    np.save('Result/sub_sigma_both_offset_{}.npy'.format(offset_error), sub_sigma_1)
    np.save('Result/sub_sigma_back_offset_{}.npy'.format(offset_error), sub_sigma_2)
    np.save('Result/sub_sigma_ref_offset_{}.npy'.format(offset_error), sub_sigma_3)
    print('Success')
...
def science(TT, ll, gain, n_pix):
    ...
    D_sci_ijk = np.array([rd.normal((tra[kk]+zodi[ll])/n_pix+Q,
                          ...
                          for kk in range(t_total[TT])], float)*gain+read*offset_error*0.01
    return D_sci_ijk
...
def background(TT, ll, gain, n_pix):
    ...
    D_back_ijk = norm.rvs(zodi[ll]/n_pix+Q,
                          np.sqrt(((bose_zodi*zodi[ll])/n_pix+Q+read)/(n_pix/n_gate)), t_total[TT])*gain+read*offset_error*0.01
    return D_back_ijk
...
def reference(TT, gain, n_pix):
    D_ref_ijk = norm.rvs(Q, np.sqrt((Q+read)/(n_pix/n_gate)), t_total[TT])*gain+read*offset_error*0.01
    return D_ref_ijk
...
def gain_calibration(nn, TT):
    ...
    return D_sub_1, D_sub_2, D_sub_3
...
def evaluation(nn, TT):
    ...
    D_sub_1, D_sub_2, D_sub_3 = gain_calibration(nn, TT)
    for ll in range(len(lamb)):
