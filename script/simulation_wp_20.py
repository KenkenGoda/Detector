...
lamb = np.load('Data/lamb_20.npy')         #wavelength(µm)
R1 = 20                                #resolution of detector1(5~10μm)
R2 = 20                                #resolution of detector2(10~20μm)
...
zodi = np.load('Data/zodi_20.npy')         #zodiacal light by wavelength(electron)
...
def main():
    ...
    #np.save('Result/wog_evaluation_20_%s.npy'%(pattern), sci_evaluation)
    np.save('Result/sci_evaluation_20_%s.npy'%(pattern), sci_evaluation)
    np.save('Result/sub_evaluation_20_%s.npy'%(pattern), sub_evaluation)
    #np.save('Result/wog_sigma_20_%s.npy'%(pattern), sci_sigma)
    np.save('Result/sci_sigma_20_%s.npy'%(pattern), sci_sigma)
    np.save('Result/sub_sigma_20_%s.npy'%(pattern), sub_sigma)
    print('Success')
...
def evaluation(nn, TT):
    global tra_one, z
    tra_one = np.load('Data/%s_20_%d.npy'%(pattern, T_star[TT]))
    z = np.load('Data/parameter_20_%d.npy'%T_star[TT])
